# Proposal v3 — rule categories, invariants, and the Rule 2 region
...
in `colregs-engine`. Supersedes v2. Reviews of v1 and v2 are in
...
|---|---|---|---|---|---|
| `definition` | 3, 21, 22 | — | vocabulary | n/a | traceability |
| `scope` | 1(a), 4, 11, 19(a), 20(b)–(c) | visibility; whether in sight; jurisdiction | which sections apply | yes | Section II gated on in-sight, III on not-in-sight; I and Part C ungated |
...
| `precedence` | 18(a)–(f), 8(f), 9(b)–(d), 10(i)–(j), 13(a) effect | two vessels' facts + encounter type | give-way / stand-on / shall-not-impede / none | yes | Alloy or Z3: never both stand-on |
| `conduct` | 6, 7(b)–(c), 8(a)–(e), 9(a),(e)–(g), 10(a)–(h),(k)–(l), 14(c), 16, 17, 19(b)–(e) | encounter + role + phase + kinematics | obligated or prohibited action | as a monitor over a trace | TLA+, STL |
...
- **1(b)–(e)** are the jurisdiction dimension already shipped (ADR-0001/2),
  not a category. **1(c)** binds `display` directly (additional lights must
  not be mistakable) and gets a drift-test line.
...
*T*; action cadence Δt; own actions bounded by the dynamics class;
...
`✎ adversary` — *A* is computed two ways from the start: other vessel
rule-compliant (the rules' own failures) and other vessel arbitrary within
physics (the Rule 17(b) world, give-way has already failed). That gives a
2×3 grid per situation. Which cells humans want to look at is what settles
it.

**Ink on tools:** TLC finds candidates (one compliant trace that collides).
Region membership is a two-player timed game and needs a game solver
(UPPAAL TIGA/STRATEGO); KeYmaera X for the few cases that need real
dynamics. Candidate traces, rendered as vessels, are what humans study
(epic M4).
...
**Ink, outputs in the region.** When *s* is in R1 or R2 the engine returns:

1. a Rule 2 banner: `status: rule-2-required` (R1) or
   `status: unwinnable-in-model` (R2), citing 2(b);
2. the rule-derived obligations, unchanged, so the reader sees what the
   rules said;
3. **advisories**: the safe escape strategies the solver found, possibly
   none, each with the paragraphs it breaks and its margin. An advisory is
   information, not a prescription. Silence here would be the wrong answer.
4. model version, dynamics class, horizon, information assumption. This is
   provenance (epic goal 4), not hedging.

Outside the region: `status: not-flagged`, meaning "not flagged by this
model", never "rules sufficient".

**Ink, the guard rail.** Obligations and advisories are functions of the
situation and the model alone. No human assertion is an input. With `O(s,h)`
the output under human input `h` and `π` dropping labels:
`∀s ∀h h′ · π(O(s,h)) = π(O(s,h′))`. Discharged by construction. Label
alphabet fixed: `{not-flagged, rule-2-required, unwinnable-in-model}`.
...
- **violation cost** — paragraphs the cheapest escape breaks (R1 only).
- **counterintuitive action** — the winning set excludes the canonical
  actions: alteration to starboard, reduction of speed.
...
- Five reviews of v1 and v2: three Claude subagents (formal methods;
