...
/** `category` defaults to 'category:display' (colregs 0.2.0, REQ-CAT-1); see header point 4. */
...
export function buildEncoding(data: ApplicabilityData): Encoding {
  ...
  for (const e of displayEntries) {
    ...
    if (e.modality === 'modality:conditional' && e.modality_by && e.modality_by.length > 0) {
      ...
      e.modality_by.forEach((b, i) => {
        ...
        if (b.modality === 'modality:shall') shallBranches.push(BRANCH(e.id, i));
        earlier.push(own);
      });
    ...
    } else if (e.modality === 'modality:conditional') {
      ...
      lines.push(
        `(define-fun ${SHALL(e.id)} () Bool ${e.modality === 'modality:shall' ? APPLIES(e.id) : 'false'})   ; modality: ${e.modality}`,
      );
    }
  }
