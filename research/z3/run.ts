...
export function parseNumeral(text: string): number {
  ...
  if (!Number.isFinite(n)) {
    ...
    // cannot produce one, so this means the theory is not what we think.
...
function snapRecord(axes: Axis[], facts: FactRecord): FactRecord {
...
function offGridAxes(axes: Axis[], facts: FactRecord): string[] {
  const out: string[] = [];
  for (const axis of axes) {
    if (axis.kind !== 'numeric') continue;
