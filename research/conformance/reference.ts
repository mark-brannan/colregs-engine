...
import type { ApplicabilityData, Entry, FactRecord, Modality, Predicate } from '../../src/types';
...
function scalarSatisfies(value: string | number | boolean, constraintValue: unknown): boolean {
...
function baseSatisfies(value: string | number | boolean, constraint: unknown): boolean {
...
/** Ids of the lights entries whose `when` holds for this fact record —
 * the reference-implementation counterpart to src/evaluate.ts's
 * appliedEntries(). */
export function referenceAppliedEntries(data: ApplicabilityData, facts: FactRecord): string[] {
  return data.entries
    .filter(isLightsEntry)
    .filter((e) => referenceWhenMatches(e.when, facts))
    .map((e) => e.id);
}
...
export function referenceModalities(
  ...
  for (const e of data.entries.filter(isLightsEntry)) {
