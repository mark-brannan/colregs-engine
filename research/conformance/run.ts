...
import { evaluate, predicateMatches } from '../../src/evaluate.ts';
import type { ApplicabilityData, Entry, FactRecord } from '../../src/types';

import { extractAxes, enumerateRecords, totalRecords, formatAxisTable } from './enumerate.ts';
import { referenceAppliedEntries, referenceResolveModality } from './reference.ts';
import { unresolvedCite, type RulesData } from './traceability.ts';
import { describeVessel } from './prose.ts';
...
const lightsEntries: Entry[] = data.entries.filter((e) => e.subjects !== 2);
const byId = new Map<string, Entry>(data.entries.map((e) => [e.id, e]));

/** Ordered pairs of lights entries where one `rel:excludes` the other. The
...
for (let i = 0; i < lightsEntries.length; i++) {
  for (let j = i + 1; j < lightsEntries.length; j++) {
    const a = lightsEntries[i];
    const b = lightsEntries[j];
...
// ---------------------------------------------------------------------
const { axes, undeclaredEnumValues } = extractAxes(data, factsJson as any);
...
for (const facts of enumerateRecords(axes)) {
  ...
  const evalResult = evaluate(data, facts as unknown as FactRecord);
  const engineApplied = evalResult.applied;
  const refApplied = referenceAppliedEntries(data, facts as unknown as FactRecord);
  ...
  if (!sameSet) {
    ...
    record(
      ...
      facts as unknown as FactRecord,
    );
  ...
  if (sameSet) {
    for (const id of engineApplied) {
      ...
      const refM = referenceResolveModality(byId.get(id)!, facts as unknown as FactRecord);
      if (engineM !== refM) {
        modalityMismatches++;
        record(
  ...
  for (const id of engineApplied) {
    everApplied.add(id);
    trackModalityByBranch(byId.get(id)!, facts as unknown as FactRecord);
  }
  ...
  if (engineApplied.length === 0) {
    noObligationCount++;
    const pos = String((facts as FactRecord)['fact:position'] ?? '(absent)');
...
// ---------------------------------------------------------------------
const neverFired = lightsEntries.filter((e) => !everApplied.has(e.id));
