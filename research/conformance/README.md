...
`enumerate.ts` reads every `when` a lights entry can gate on — the entry's
own, each `modality_by[].when`, and any `when` under
`rel:conditional_includes` — and collects the fact keys those predicates
read. Facts no predicate reads are left absent, because the evaluator never
looks at them.
...
|---|---|
| enum | every value declared in `facts.json` |
| boolean | `true`, `false` |
...
`fact:length_m` has eleven representatives because the lights entries compare
...
**coverage** — lights entries that never apply anywhere in the space,
`modality_by` branches that are never the first match, and `one_of` options
that are never selectable. All three are currently empty, which is the
result you want: every entry and every branch is reachable.

**traceability** — every lights entry's `cite` resolves to a paragraph in
colregs `data/rules.json`, including both ends of a range cite like
`23(a)(iii)-(iv)`.
