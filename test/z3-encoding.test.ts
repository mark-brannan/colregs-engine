...
describe('the encoding against Z3', () => {
  ...
  it('defines `shall:` as evaluate.ts resolves the modality, for every modality', async () => {
    // Was FIND-01's facts; that finding is regenerated by the conformance
    // harness against the pinned colregs data and its content is no longer
    // stable across versions (colregs 0.2.2 reshaped it into the
    // 30a-buoy/30b-buoy mooring case, position:moored rather than anchored,
    // so 30a's `when` no longer matched). A record that plainly satisfies
    // Rule 30's `position:anchored` gate, independent of any finding.
    const base: Record<string, FactValue> = {
      'fact:activity': 'activity:none',
      'fact:composite_unit': false,
      'fact:gear_extent_m': 0,
      'fact:length_m': 6,
      'fact:making_way': false,
      'fact:manned': true,
      'fact:max_speed_kn': 0,
      'fact:near_channel': false,
      'fact:nearing_obscured_bend': false,
      'fact:non_displacement': false,
  ...
  // colregs@0.2.2 settled FIND-01/FIND-02 (26(a) now rel:overrides, not
  // rel:excludes): expectations.json keeps their entries as historical
  // record but no longer points them at a `finding`, so `pointed` is
  // empty until a new one is recorded. Skipped rather than deleted so it
  // comes back the moment expectations.json names one again.
  it.skipIf(
    Object.values(loadExpectations().queries).every((r) => !r.finding),
  )('is satisfied by the representative record of every finding it expects', async () => {
